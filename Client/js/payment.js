...
function makeItems() {
  ...
  xmlHttp.onreadystatechange = function() {
      ...
      if (xmlHttp.status == 500 || obj.status == "table not seated"){
        console.log("error");
      }
          ...
          for (var i = 0; i < numItems; i++){
            ...
            name.setAttribute("class", "col-md-9 text-left");
            var em = document.createElement("em");
            em.innerHTML = obj.items[i].name;
            name.appendChild(em);
            var quantity = document.createElement("td");
            quantity.setAttribute("class", "col-md-1");
            quantity.setAttribute("style", "text-align: left");
  ...
  xmlHttp.open("GET", "http://waiter.bleak.io/cartSum/12/-2", true);
  xmlHttp.send(null);
